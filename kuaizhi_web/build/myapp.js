...
var Topmenu=React.createClass({displayName: "Topmenu",
	...
	render: function() {
		...
		var self=this;
		return ( 
			React.createElement("ul", {className: "mymenu"}, 
			    React.createElement("li", {className: "mymenu-active", onClick: self.handleClickBack}, "最新"), 
			    s.map(function(result,k) {
			        return React.createElement("li", {id: result.id, onClick: self.handleClickBack}, result.title)
				}), 
			    React.createElement("li", {id: "520", onClick: self.handleClickBack}, "剪报")
			)
		);
	}
});
...
var Cardlist=React.createClass({displayName: "Cardlist",
    ...
    componentDidMount: function() {
    	...
    	EventEmitter.subscribe('changeItem', function(newItem){
			...
			//id=newItem;
			if(id==520){//跳转到剪报
				React.render(React.createElement(Likelist, null),
	...
	render: function() {
		...
		return (
			...
			q.map(function (result,k) {  
				return  React.createElement("li", {"data-id": result.id, className: "mui-card mycard"}, 
							React.createElement("div", {className: "mui-collapse-content mui-text-right"}, 
								React.createElement("span", {className: "mui-spinner mui-pull-left likebtn", onClick: self.likedcallback}), 
								React.createElement("span", {className: "mui-icon mui-icon-close", onClick: self.removedcallback})
							), 
							React.createElement("div", {className: "mui-collapse-content mycard-text"}, 
								React.createElement("p", null, result.content)
							), 
							React.createElement("div", {className: "mui-collapse-content mycard-date"}, 
								React.createElement("h5", null, React.createElement("span", {className: "mui-badge mui-badge-primary"}, result.objname), "  ", React.createElement("span", null, result.created))
							)
						)
			})
			)
...
//收藏列表
var Likelist=React.createClass({displayName: "Likelist",
	getInitialState: function() {
	    return {
	        cards: [{
	        	'id':"1",
    ...
    componentDidMount: function() {
    	...
    	EventEmitter.subscribe('changeItem', function(newItem){
			...
			if(id==520){//跳转到剪报
				...
				React.render(React.createElement(Cardlist, {source: "http://api.kuaizhi.imgrids.com/api/index/article_list.html?callback=?"}),
					document.getElementById('cardlists')
				);
			}
		});
	...
	render: function() {
		...
		return (
			...
			q.map(function (result,k) {  
				return  React.createElement("li", {"data-id": result.id, className: "mui-card mycard"}, 
							React.createElement("div", {className: "mui-collapse-content mui-text-right"}, 
								React.createElement("span", {className: "mui-icon mui-icon-star-filled myicon  likebtn", onClick: self.likedcallback})
							), 
							React.createElement("div", {className: "mui-collapse-content mycard-text"}, 
								React.createElement("p", null, result.content)
							), 
...
//日期选择
var Datemenu=React.createClass({displayName: "Datemenu",
	getInitialState: function() {
      return {
	...
	handleClickBack: function(event) {//日期选择
		...
		var optionsJson = '{"type":"date","beginYear":'+fy+',"endYear":'+fy+'}';
		var options = JSON.parse(optionsJson);
		var picker = new mui.DtPicker(options);
		picker.show(function(rs) {
			dateNow=rs.text;
			picker.dispose();
			EventEmitter.dispatch('changeItem',dateNow);
	...
	render: function() {
		...
		var self=this;
		return ( 
			React.createElement("div", {className: "mui-slider"}, 
				React.createElement("div", {className: "mui-slider-group"}, 
				   s.map(function(result,k) {
				    return React.createElement("div", {className: "mui-slider-item datenote", onClick: self.handleClickBack}, 
				           "  ", result.content
				           )
				    })
				)
...
React.render(React.createElement(Topmenu, {source: "http://api.kuaizhi.imgrids.com/api/index/article_list.html?callback=?"}),
	document.getElementById('mymenu')
);
React.render(React.createElement(Cardlist, {source: "http://api.kuaizhi.imgrids.com/api/index/article_list.html?callback=?"}),
	document.getElementById('cardlists')
);
React.render(React.createElement(Datemenu, null),
